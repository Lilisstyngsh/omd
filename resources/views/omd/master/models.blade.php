...
@section('content')
    <style>
        ...
        .context-head { display:flex; justify-content:space-between; align-items:center; gap:16px; flex-wrap:wrap; margin-bottom:20px; }
        .context-title h2 { margin:0; }
        .context-title p { margin:6px 0 0; color:#667085; }
        .master-grid-forms { display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:20px; }
        .context-chip { display:inline-flex; padding:6px 10px; border-radius:999px; background:#f5f3ff; color:#6d28d9; font-size:11px; font-weight:750; margin-top:10px; }
        .master-table th,.master-table td { border:1px solid #e9e7ef; vertical-align:middle; }
        .master-table th { background:#faf9fc; }
        .action-inline { display:inline-flex; gap:6px; flex-wrap:wrap; justify-content:center; }
        .btn-sm { padding:7px 10px; font-size:12px; }
        .col-no { width:70px;text-align:center; }
        .col-model { width:180px; }
        .col-product-no { width:80px;text-align:center; }
        .col-action { width:190px;text-align:center; }
        @media (max-width:800px) { .master-grid-forms { grid-template-columns:1fr; } }
    </style>
            ...
            <table class="table master-table">
                ...
                <tbody>
                    @forelse ($models as $model)
                        ...
                        @endforelse
                    @empty
                        <tr><td colspan="5" class="empty">Belum ada Model pada area ini.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
